...
extern int vx_dump_perf(vx_device_h device, FILE* stream) {
  ...
  for (unsigned core_id = 0; core_id < num_cores; ++core_id) {
    ...
    if (num_cores > 1) {
      fprintf(stream, "PERF: RYAN179: core%d: active threads=%ld\n", core_id, active_threads_per_core);
    } 
  ...
  // assignment 1
  // divide total active threads by cycle count and print # active threads per cycle
  double active_threads_per_cycle = (double(active_threads) / double(cycles));
  fprintf(stream, "PERF: RYAN322: active threads per cycle=%f\n", active_threads_per_cycle);
